...
### Step 5. DOM 요소를 직접 생성하기 (createElement)

1. 아래 코드를 작성하고 새로고침한다

```javascript
const h2 = document.createElement("h2");
h2.textContent = "createElement로 만든 요소";
document.body.appendChild(h2);
```

이 코드는 **HTML 문자열을 직접 넣는 방식(innerHTML)** 이 아니라,
JavaScript로 **DOM 요소 객체를 생성(create)** 한 뒤 화면에 추가하는 예제다.

* **`document.createElement("h2")`**
  `<h2>` 태그에 해당하는 **DOM 요소 객체를 메모리에 생성**한다.
  이 단계에서는 아직 화면에 보이지 않는다.

* **`h2.textContent = "createElement로 만든 요소"`**
  생성한 `<h2>` 요소에 **텍스트 내용**을 설정한다.

* **`document.body.appendChild(h2)`**
  생성된 DOM 요소를 `<body>`에 **연결**한다.
  DOM 트리에 포함되는 순간 브라우저가 이를 인식하고 **화면에 표시한다**.

* DOM 요소는 문자열이 아니라 **객체**다
* `createElement`는 요소를 **만드는 단계**, `appendChild`는 **화면에 연결하는 단계**다
...
> JavaScript는 실행 환경 위에서, DOM 생성 이후에, 위에서 아래로 실행되며    
DOM을 바꾸는 방식으로 화면에 영향을 준다.
